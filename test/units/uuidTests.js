...
suite('uuidv4', () => {
  ...
  suite('regex', () => {
    ...
    suite('v4', () => {
      test('is a regular expression that describes a UUID v4.', async () => {
        assert.that(uuidv4.regex.v4).is.equalTo(/^([a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[a-f0-9]{4}-[a-f0-9]{12})|(0{8}-0{4}-0{4}-0{4}-0{12})$/);
      });
    });

    suite('v5', () => {
      test('is a regular expression that describes a UUID v5.', async () => {
        assert.that(uuidv4.regex.v5).is.equalTo(/^([a-f0-9]{8}-[a-f0-9]{4}-5[a-f0-9]{3}-[a-f0-9]{4}-[a-f0-9]{12})|(0{8}-0{4}-0{4}-0{4}-0{12})$/);
      });
    });
  });
  ...
  suite('is', () => {
    ...
    test('returns true if an empty UUID is given.', async () => {
